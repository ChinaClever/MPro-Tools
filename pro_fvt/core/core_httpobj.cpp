...
void Core_HttpObj::setting(const sDataItem &it, const QString &ip, int port)
{
    ...
    http_post("pduSetting", obj, ip, port);
}

void Core_HttpObj::setting(const sCfgItem &it, const QVariant &value, const QString &ip, int port)
{
    QJsonObject json, obj;
    json.insert("addr", it.addr);
    json.insert("type", it.type);
...
void Core_HttpObj::uploadFile(const QStringList fs, const QString &ip, int port)
{
    foreach (auto fn, fs) {
        http_upload("upload", fn, ip, port);
    }
}

void Core_HttpObj::downFile(const QStringList fs, const QString &ip, int port)
{
    foreach (auto fn, fs) {
        QJsonObject json;
        json.insert("file", fn);
        http_down("download", json, fn, ip, port);
    }
}
...
void Core_HttpObj::http_upload(const QString &method, const QString &file, const QString &ip, int port)
{
    ...
    mHttp.post(url.arg(ip).arg(port).arg(method)).body(file, file)
            .onSuccess([&](QString result) {emit httpSig("Upload completed: "+file +result);})
    .onFailed([&](QString error) {emit httpSig("Upload failed: "+error); })
    .sslConfiguration(sslConfig())
            .timeout(1000) // 1s超时
            .exec();
}
